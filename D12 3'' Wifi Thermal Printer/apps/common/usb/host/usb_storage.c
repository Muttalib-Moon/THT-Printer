...
static int _usb_stor_read_big_block(struct device *device, u32 disk_block_size, void *pBuf, u8 num_lba, u32 lba)
{
    ...
    u8 req_offset = lba % num_block; //计算block数据偏移�?
    u8 send_cbw = 0;
    if ((req_offset == 0)  || (lba <= disk->prev_lba) || ((lba - disk->prev_lba) > num_block) || (disk->remain_len == 0)) {
        //sector 4k地址;或地�?不连续时(地址小于上一次地�?; 地址和上�?次地�?不属于同�?个sector)
        send_cbw = 1;
    } else {
        //地址连续
        send_cbw = 0;
    ...
    if (send_cbw) {
        _usb_stor_read_block_finish(device, disk->remain_len, num_lba, pBuf); //确保读完剩余�? */
        disk->remain_len = 0;
        disk->dev_status = DEV_READ;
        disk->suspend_cnt = 0;
        ...
        req_offset = 0; //地址连续�?,读一次data
    }
    //data
    do {
        //循环读取,直到读取到偏移量�?在的数据为止
        ret = usb_bulk_only_receive(device,
                                    disk->udisk_ep.host_epin,
                                    rxmaxp,
                                    disk->udisk_ep.target_epin,
    ...
    if (!disk->remain_len) { //读完data,�?要读csw
        //csw
        ret = usb_bulk_only_receive(device,
                                    disk->udisk_ep.host_epin,
                                    rxmaxp,
...
static int _usb_stor_read_ahead_cbw(struct device *device, void *pBuf, u32 num_lba, u32 lba)
{
    ...
    u8 read_block_num = UDISK_READ_AHEAD_BLOCK_NUM; //�?要预读扇区数�?
...
static int _usb_stor_read_ahead(struct device *device, void *pBuf, u32 num_lba, u32 lba)
{
    ...
    if ((lba != disk->udisk_read_ahead_lba_last + 1) || disk->remain_len == 0) {
        ret = _usb_stor_read_ahead_data(device, NULL, disk->remain_len / 512, lba); //预读中止，把上一次的剩下的读�?
        check_usb_read_ahead(ret);
...
/**
 * @brief usb_stor_read 从U盘的lba扇区读取num_lba个扇�?
 *
 * @param device 设备句柄
 * @param pBuf 读buffer缓冲区，芯片�?有memory都可�?
 * @param lba �?要读取的扇区地址
 * @param num_lba �?要读取的扇区数量
 *
 * @return 负数表示失败
 * 			大于0的表示读取的字节�?(Byte)
 */
...
/**
 * @brief usb_stor_write 写数据到U�?
 *
 * @param device
 * @param pBuf 数据buffer
 * @param lba   �?要写入的扇区地址
 * @param num_lba �?要写入的扇区数量
 *
 * @return 负数表示写入失败
 * 		正数为写入的字节�? (Byte)
 */
static int _usb_stor_write(struct device *device, void *pBuf,  u32 num_lba, u32 lba)
{
    ...
    if (disk->remain_len) {
        _usb_stor_read_ahead_data(device, NULL, disk->remain_len / 512, disk->udisk_read_ahead_lba_last + 1); //预读中止，把上一次的剩下的读�?
 ...
 * 			|命令|功能|
 * 			|---|---|
 * 			|IOCTL_GET_ID|U盘位于�?�线的地�?0表示不在线|
 * 			|IOCTL_GET_BLOCK_SIZE|获取扇区大小|
 * 			|IOCTL_GET_CAPACITY|获取磁盘容量|
 * 			|IOCTL_GET_MAX_LUN|获取�?大磁盘数�?<�?大支持两个磁�?>|
...
#define BLOCKS_TO_KICKSTARTS    64  //多少块统计一次时�?
