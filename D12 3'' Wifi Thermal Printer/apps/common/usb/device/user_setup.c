...
u8 DeviceIDPrinterStr[]="1MANUFACTURER:NOBLE PARAGON;MODEL:TP870 SZ;ACTIVE COMMAND:TSPL";
static u32 setup_device(struct usb_device_t *usb_device, struct usb_ctrlrequest *req)
{
    ...
    case USB_REQ_SET_ADDRESS:
       ...
       case 0:
        {
                unsigned int x=0;
               if(req->bRequestType==0xa1)
               {
               printf("sang:GET DEVICE ID OK--------------------\r\n");
               u8 DeviceIDPrinter[512]={0};
               DeviceIDPrinter[0]=0;
             //  u8 DeviceIDPrinterStr[]="1MANUFACTURER:NOBLE PARAGON;MODEL:TP870 S;ACTIVE COMMAND:TSPL";

               for(x=0;x<strlen(DeviceIDPrinterStr);x++)
               {
                    DeviceIDPrinter[1+x]=DeviceIDPrinterStr[x];
               }
                usb_set_data_payload(usb_device, req, DeviceIDPrinter, x+1);
               }
               ret = 1;
        }
        break;
...
static u32 user_setup_filter(struct usb_device_t *usb_device, struct usb_ctrlrequest *request)
{
    ...
    //u32 recip = request->bRequestType & USB_RECIP_MASK;
       ///----modify by sang
    u32 recip = request->bRequestType;
    switch (recip)
    {
