...
u16 Page_CutPosition;
u16 Page_CutContinue;
bool Page_CutFlag;
...
/*
 * 测试cpu0,cpu1 stack用多少空�?
 * 使用list_for_stack_info查看stack使用空间前，先在main函数�?先调用init_for_stack_info初始化stack
 */
...
void cpu1_main(void)
{
    ...
    //在这句话之后 不可用操作系统接�?,printf/puts等打印接�?,并且�?要替换单核专用的system�?,
    while (1) {
...
static void rtos_stack_check_func(void *p)
{
    ...
    get_task_state(NULL); //1分钟以内调用�?次才准确
#endif
...
#if CPU_CORE_NUM == 1
    EnableOtherCpu();
    //cpu1_run_flag = 0; //如果运行在SFC模式�?,不希望写FLASH的时候会挂起CPU1,则打�?这句�?,但是�?要保证cpu1_main调用到的�?有函数全部放到内部ram或�?�sdram,防止写flash过程中引起死�?
#endif
...
#if CPU_CORE_NUM == 1 //本来�? prvIdleTask
    __local_irq_enable();
#endif
...
{
    /* 系统进入在未来时间里，无任务超时唤醒，可根据用户选择系统停止，或者系统定时唤�?(100ms) */
    //1:Endless Sleep
    //0:100 ms wakeup
    return 0;
}
...
int cpu_irq_disabled(void)
{
    ...
    //只读�?遍有可能出现实际上没关中断但又条件成立的情况，需要再读一次确保正确，读取前使用ssync也没效果，原因未�?
