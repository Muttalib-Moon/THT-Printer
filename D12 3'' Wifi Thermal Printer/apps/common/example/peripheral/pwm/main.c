...
/*********************************PWM设备例子****************************************************
  ...
  PWM通过dev_read读，返回值为通道的占空比�?(0-100%)�?
  PWM通过dev_ioctl控制，形参arg为PWM通道�?PWM control dev_ioctl command to set the device to
************************************************************************************************/
...
static void pwm_test(void)
{
    ...
    /*1.open ，open之后的PWM通道�?多支持同�?个�?�道的H和L设置，不能进行多通道的设�?*/
    pwm_dev_handl = dev_open("pwm1", &pwm);//打开PWM0设备，传参可以获取board.c配置的参�?
    if (!pwm_dev_handl) {
        printf("open pwm err !!!\n\n");
        return ;
    }
    ...
    /*open PWM设备之后就会初始化PWM，PWM相关参数为board.c配置，在不需要更改参数时，只�?要open就行，不�?要进行以下操�?*/

#if 1
    /*2.write and read 配置占空�?*/
    pwm.pwm_ch = PWMCH0_H | PWMCH0_L;//该�?�道�?在board.c中有定义，若没有则需先PWM_ADD_CHANNEL,下面有例子�??
    pwm.duty = 80;
    ...
    /*3.ioctl控制PWM暂停、运行�?�正反向,调用1次ioctl只支�?1组�?�道PWMCH_H/L控制*/
    printf("----pwm0 ioctl-------\n\n");
    ...
    pwm.deathtime = 6;//�?大�??31 死区时间为系统时钟的(deathtime+1)�?,使用PWMCHx_H/L有效
    ...
    pwm.duty = 50;
    dev_ioctl(pwm_dev_handl, PWM_SET_DUTY, (u32)&pwm);//设置占空�?
    os_time_dly(200);
    ...
    /*5.中�?�可以添加TIMER2 PWM 任意IO,添加通道后关闭前必须删除 */
    printf("----timer add channel-------\n\n");
    pwm.pwm_ch = PWM_TIMER2_OPCH2;
    pwm.port = IO_PORTA_07;
    pwm.duty = 10;
    pwm.freq = 1500;
    dev_ioctl(pwm_dev_handl, PWM_ADD_CHANNEL, (u32)&pwm);//中�?�添加�?�道，可以是PWMCHx_H/L和PWM_TIMER2_OPCH2或PWM_TIMER3_OPCH3

    pwm.duty = 80;
    dev_write(pwm_dev_handl, (void *)&pwm, 0);//dev_write也可以设置占空比
    ret = dev_read(pwm_dev_handl, (void *)&pwm, 0);//读取占空�?
    printf("pwm0 read duty : %d \n", ret);
    ...
    /*6.ioctl控制PWM暂停、运行�?�正反向,调用1次ioctl只支�?1组�?�道PWMCH_H/L控制*/
    os_time_dly(300);
    dev_ioctl(pwm_dev_handl, PWM_STOP, (u32)&pwm);//PWM停止
