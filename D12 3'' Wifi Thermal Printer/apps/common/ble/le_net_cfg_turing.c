...
#define ATT_SEND_CBUF_SIZE        (512)                   //note: need >= 20,ÁºìÂ≠òÂ§ßÂ∞èÔºåÂèØ‰øÆÊîπ
...
// Complete Local Name  ÈªòËÆ§ÁöÑËìùÁâôÂêçÂ≠?
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    log_info("999read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);

    switch (handle) {
#if 1	 //------------------------------------------------------@20240224 ATT_Sang À´œÚ≤‚ ‘ Ãÿ’˜◊÷∑˚¥Æø™ º-------------------------------------------------------
	case ATT_CHARACTERISTIC_FEC9_01_VALUE_HANDLE:
		/////–Ë“™∑µªÿµƒ≥§∂»
	  const u8 test_read_buf_sangzijin[5]={'s','a','n','7',0x55};
...
#define PASSKEY_ENTER_ENABLE      0 //ËæìÂÖ•passkey‰ΩøËÉΩÔºåÂèØ‰øÆÊîπpasskey
//ÈáçËÆæpasskeyÂõûË∞ÉÂáΩÊï∞ÔºåÂú®ËøôÈáåÂèØ‰ª•ÈáçÊñ∞ËÆæÁΩÆpasskey
//passkey‰∏?6‰∏™Êï∞Â≠óÁªÑÊàêÔºåÂçÅ‰∏á‰Ωç„?Å‰∏á‰Ωç„?Ç„?Ç„?Ç„?Ç‰∏™‰Ω? ÂêÑË°®Á§∫‰∏Ä‰∏™Êï∞Â≠? È´ò‰Ωç‰∏çÂ§ü‰∏?0
static void reset_passkey_cb(u32 *key)
{
#if 1
    u32 newkey = rand32();//Ëé∑ÂèñÈöèÊú∫Êï?

    newkey &= 0xfffff;
    if (newkey > 999999) {
        newkey = newkey - 999999; //‰∏çËÉΩÂ§ß‰∫é999999
    }
    *key = newkey; //Â∞è‰∫éÊàñÁ≠â‰∫éÂÖ≠‰ΩçÊï∞
