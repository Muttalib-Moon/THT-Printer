...
struct pair_info_t {
    ...
    u8  direct_adv_flag: 1;   //是否�?要定向广�?
    u8  direct_adv_cnt: 5;    //定向广播次数
    u8  peer_address_info[7]; //绑定的对方地�?信息
    u16 tail_tag;//尾标�?
};
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    log_info("5555read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);

    switch (handle) {
#if 1 //@20240319 SangStoneAngleAlke close
	 //------------------------------------------------------@20240224 Sang ˫����� �����ַ�����ʼ-------------------------------------------------------
	case ATT_CHARACTERISTIC_FEC9_01_VALUE_HANDLE:
		/////��Ҫ���صĳ���
	  const u8 test_read_buf_sangzijin[5]={'s','a','n','1',0x55};
	  att_value_len = sizeof(test_read_buf_sangzijin);
...
//------------------------------------------------------
static u8 adv_name_ok = 0; //名字优先放入adv�?
...
{
    ble_list_config_reset(pair_max, is_allow_cover); //�?1�?1配对绑定
}
