...
#include "le_common.h"
#include "../wifi_story_machine/bt_ble/BTPrinter.h"
#include "AppDriverGeneric.h"//JL lib dependency
#include "AppDriver.h"
#include "../../../include_lib/net/lwip_2_1_3/lwip/port/lwip.h"

extern void BLEPrinterReceiveDataISR(unsigned char *data,unsigned int len);
...
char gap_device_name[BT_NAME_LEN_MAX] = "wl80_ble_test";
u8 gap_device_name_len = 0;
...
unsigned char Status[3]="000";//bit 0:0:cover close,1:cover open;bit2:0:paper ok,1:no paper;bit3:0:receive buffer ok,1:recieve buffer full
extern bool BleBusy;
extern PDataPacket PBLEDP;
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
  ...
  //log_info("666read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);

  switch (handle)
  {
	case ATT_CHARACTERISTIC_FEC9_01_VALUE_HANDLE:
	 /* att_value_len = sizeof(Status);
	  printf("Read printer status offset:%d,att_value_len:%d>>",offset,att_value_len);
      if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len)
      {
        break;
      }
      ...
      if (buffer)
      {
        ...
        memcpy(buffer, &Status[offset], buffer_size);
        att_value_len = buffer_size;
      }
      */

        u8 test_read_buf_sangzijin[7] = {'Z','0','0','0','0','0', 0};

        u16 c16c1 = 0;
        u16 nodeCount = _malloc_size(PBLEDP);

        if (nodeCount < 50)
            c16c1 = 1000 - nodeCount;
        else
            c16c1 = 0;

       printf("get NODE count:: %d calculated nodcount %d : \n",nodeCount,c16c1 );

        int c8cc = 0;
        int c8d1 = 0;
        test_read_buf_sangzijin[c8cc++] = 'Z';

        c16c1 %= 100000;
        if ((c16c1 / 10000) || c8d1) { test_read_buf_sangzijin[c8cc++] = (u8)(c16c1 / 10000) + '0'; c8d1 = 1; }
        c16c1 %= 10000;
        if ((c16c1 / 1000) || c8d1) { test_read_buf_sangzijin[c8cc++] = (u8)(c16c1 / 1000) + '0'; c8d1 = 1; }
        c16c1 %= 1000;
        if ((c16c1 / 100) || c8d1)  { test_read_buf_sangzijin[c8cc++] = (u8)(c16c1 / 100) + '0';  c8d1 = 1; }
        c16c1 %= 100;
        if ((c16c1 / 10) || c8d1)   { test_read_buf_sangzijin[c8cc++] = (u8)(c16c1 / 10) + '0';   c8d1 = 1; }
        c16c1 %= 10;
        if ((c16c1) || c8d1)        { test_read_buf_sangzijin[c8cc++] = (u8)(c16c1) + '0';        c8d1 = 1; }
        while (c8cc < 7) test_read_buf_sangzijin[c8cc++] = 0;

        att_value_len = (u16)strlen((char *)test_read_buf_sangzijin);
      //  printf("Read printer buffer offset:%d, att_value_len:%d >>", offset, att_value_len);
        if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len)
            break;
        if (buffer)
        {
           //printf("return buffer info: %s\r\n", test_read_buf_sangzijin);
            memcpy(buffer, &test_read_buf_sangzijin[offset], buffer_size);
            att_value_len = buffer_size;
        }
	  break;
    ...
    case ATT_CHARACTERISTIC_2A24_01_VALUE_HANDLE:
        ...
        extern char SSIDSave[64];
        u8 ssid_buf[64] = {0};

        GetWifiSTAInfo();
        snprintf((char *)ssid_buf, sizeof(ssid_buf), "SSID:%s", SSIDSave);

        att_value_len = strlen((char *)ssid_buf);
        ...
        if (buffer) {
            memcpy(buffer, &ssid_buf[offset], buffer_size);
            att_value_len = buffer_size;
        }
        break;

    case ATT_CHARACTERISTIC_2A27_01_VALUE_HANDLE:
		u8 test_read_buf[18]={"IP::"};
		//unsigned long IPAddress = 0;
		extern unsigned long IPAddress;

		struct netif_info netif_infoA;
		lwip_get_netif_info(1, &netif_infoA);

		IPAddress = netif_infoA.ip;
		u8 c8cc_ip = 3;
		u8 flag = 0;
		u8 c8d1_ip = IPAddress&0x000000ff;
		if(c8d1_ip > 99)
			{test_read_buf[c8cc_ip++] = 0x30+c8d1_ip/100; flag = 1;}
		c8d1_ip %= 100;
		if(c8d1_ip > 9)
			test_read_buf[c8cc_ip++] = 0x30+c8d1_ip/10;
		else if(flag)
			test_read_buf[c8cc_ip++] = 0x30;
		c8d1_ip %= 10;
		test_read_buf[c8cc_ip++] = 0x30+c8d1_ip;
		test_read_buf[c8cc_ip++] = '.';

		flag = 0;
		c8d1_ip = (IPAddress>>8)&0x000000ff;
		if(c8d1_ip > 99)
			{test_read_buf[c8cc_ip++] = 0x30+c8d1_ip/100; flag = 1;}
		c8d1_ip %= 100;
		if(c8d1_ip > 9)
		...
		flag = 0;
		c8d1_ip = (IPAddress>>16)&0x000000ff;
		if(c8d1_ip > 99)
			{test_read_buf[c8cc_ip++] = 0x30+c8d1_ip/100; flag = 1;}
		c8d1_ip %= 100;
		if(c8d1_ip > 9)
		...
		flag = 0;
		c8d1_ip = (IPAddress>>24)&0x000000ff;
		if(c8d1_ip > 99)
			{test_read_buf[c8cc_ip++] = 0x30+c8d1_ip/100; flag = 1;}
		c8d1_ip %= 100;
		if(c8d1_ip > 9)
		...
		att_value_len = sizeof(test_read_buf);//(test_read_write_buf);
		if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len)	break;
		if (buffer)
		{
			memcpy(buffer, &test_read_buf[offset], buffer_size);
			att_value_len = buffer_size;
		}
		break;
  ...
  log_info("write_callback, handle= 0x%04x,size = %d\n", handle, buffer_size);
		  ...
		  switch (handle)
		  {
		    ...
		    case ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE:
				/*printf("Characteristic FF02: Data= ");
		            for (int i = 0; i < buffer_size; i++)
		            {
		                printf("%02x ", buffer[i]);  // Print buffer content in hex
		            }
		            printf("\n");*/
		            /*----------------------->
		     if(Data_Channel!=LEData)
		        {Data_Channel=LEData;InputCOM_device = LE_RevC;}
		      if(LEstateS==LEFree)
		      {
		       // SaveData(buffer_size , buffer, BTData);
                    LESaveData(buffer_size , buffer);
		      }
		      if(GetUSBEmptyBuff(3)<(BufferReceiveSizeEmpty+500))//need edit
		      {
		    ...
		    case ATT_CHARACTERISTIC_49535343_8841_43F4_A8D4_ECBE34729BB3_01_VALUE_HANDLE://WRITE    | WRITE_WITHOUT_RESPONSE |DYNAMIC,
		        ...
		        {
		        	Data_Channel=LEData;InputCOM_device = LE_RevC;
					data_usb_channel_busy_flag = true;
            ...
            */
			/*if(Data_Channel !=BTData)
				 { Data_Channel=BTData; InputCOM_device = BT_RevC;}

				if(BTstateS==BTFree)
				{
					BTSaveData(buffer_size , buffer);
				}

				if(GetUSBEmptyBuff(BTData)<(BufferReceiveSizeEmpty+500))
				{
					att_server_flow_enable(1);
		         att_server_flow_hold(connection_handle,1);
					BTstateS=BTBusy;
				}*/
			...
			case ATT_CHARACTERISTIC_FF82_01_VALUE_HANDLE:
			  ...
			  if(GetEmptySize()<BusyLevelBLE&&BTI.BLEState==NormalState)
			  {
		        ...
		        */
                //att_server_flow_enable(1);
                BLEPrinterReceiveDataISR(buffer,buffer_size);
                //HCI_le_Printer=connection_handle;
...
//passkey�?6个数字组成，十万位�?�万位�?��?��?��?�个�? 各表示一个数�? 高位不够�?0
static void reset_passkey_cb(u32 *key)
{
#if 1
    u32 newkey = rand32();//获取随机�?
