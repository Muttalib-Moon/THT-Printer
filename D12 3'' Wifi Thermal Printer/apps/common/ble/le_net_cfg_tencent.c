...
#define DEFAULT_SSID_MAX_LEN		32//64		//默认ssid�?大长�?
#define DEFAULT_PWD_MAX_LEN			64		//默认pwd�?大长�?
...
//1字节长度�?1字节\0
static u8 save_ssid_info[DEFAULT_SSID_MAX_LEN + 2] = {0};     //存放ssid信息，用于信息返�?
static u8 save_pwd_info[DEFAULT_PWD_MAX_LEN + 2] = {0};     //存放password信息
//1字节\0
static u8 save_token_info[DEFAULT_TOKEN_LEN + 1] = {0};	//存放token信息

static int send_max_length;	//�?次发送最长的长度
static int BTCombo_flag = 0;		//配网步骤标志�?
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    log_info("888read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);

    switch (handle) {
#if 1	 //------------------------------------------------------@20240224 ATT_Sang ˫����� �����ַ�����ʼ-------------------------------------------------------
	case ATT_CHARACTERISTIC_FEC9_01_VALUE_HANDLE:
		/////��Ҫ���صĳ���
	  const u8 test_read_buf_sangzijin[5]={'s','a','n','6',0x55};
...
static void send_wifi_report()
{
    ...
    if (remain_len + 4 > send_max_length) {
        while (remain_len > send_len) {
            ...
            send_buf[0] = 0x11;                 //0x11 发�?�配网状态给手机
            send_buf[1] = 0x10 | 0x04;          //0x14  0x10分片 0x04 设备发数
            send_buf[2] = seq++;                //序列�?,每次�?1
            send_buf[3] = send_len + 2;         //包含剩余数据长度在内2byte的数据长�?
...
{
    int cnt = 0;
    while (!net_dhcp_ready()) {     //等待20s,�?测wifi连接状�??
        os_time_dly(20);
        cnt++;
...
void send_reply_token(char *para_format, int len)
{
    ...
    while (remain_len > send_len) {
        memset(send_buf, 0, sizeof(send_buf));
        send_buf[0] = 0x15;                 //0x15 用户发�?�token 或�?�接�? token绑定状�?�信�?
        send_buf[1] = 0x10 | 0x04;          //0x14  0x10分片 0x04 设备发数
        send_buf[2] = seq++;                //序列�?,每次�?1
        send_buf[3] = send_len + 2;         //包含剩余数据长度在内2byte的数据长�?
        send_buf[4] = remain_len % 0xff;    //剩余数据长度低位
