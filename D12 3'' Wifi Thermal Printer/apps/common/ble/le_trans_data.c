...
#define TEST_SEND_DATA_RATE          0  //测试上行发�?�数�?
...
//------
//ATT发�?�的包长,    note: 20 <=need >= MTU
#define ATT_LOCAL_MTU_SIZE    (200)                   //
//ATT缓存的buffer大小,  note: need >= 20,可修�?
...
/*
 打开流控使能�?,确定使能接口 att_server_flow_enable 被调�?
 然后使用过程 通过接口 att_server_flow_hold 来控制流控开�?
 注意:流控只能控制对方使用带响应READ/WRITE等命令方�?
...
static u8 gap_device_name_len = 0; //名字长度，不包含结束�?
static u8 ble_work_state = 0;      //ble 状�?�变�?
...
static int set_adv_enable(void *priv, u32 en); //add
static int get_buffer_vaild_len(void *priv);//add
...
{
    u32 vaild_len = get_buffer_vaild_len(0);//获取发�?�buffer可写入的数据
    if (!test_data_start) {
        return;
    }
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    log_info("454read_callback, handle= 0x%04x,buffer= %08x\n", handle, (u32)buffer);
  ...
  switch (handle)
  {
    ...
    case ATT_CHARACTERISTIC_49535343_8841_43F4_A8D4_ECBE34729BB3_01_VALUE_HANDLE:
        /////???????
	  const u8 test_read_buf_angenl[5]={'z','h','a','g',0x55};
	  att_value_len = sizeof(test_read_buf_angenl);
      if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len)
      {
        break;
      }
      ...
      if (buffer)
      {
        memcpy(buffer, &test_read_buf_angenl[offset], buffer_size);
        att_value_len = buffer_size;
      }
...
static int getNumberLength(int value) {
    int length = 0;
  ...
  switch (handle)
  {
   ...
   case ATT_CHARACTERISTIC_49535343_8841_43F4_A8D4_ECBE34729BB3_01_VALUE_HANDLE:
     /* if(Data_Channel!=LEData)
      ...
      if(GetUSBEmptyBuff(3)<(BufferReceiveSizeEmpty+500))
      {
          att_server_flow_enable(1);
         att_server_flow_hold(HCI_le_Printer,1);
        LEstateS=LEBusy;
