...
#define ATT_SEND_CBUF_SIZE        (512)                   //note: need >= 20,ç¼“å­˜å¤§å°ï¼Œå¯ä¿®æ”¹
...
// Complete Local Name  é»˜è®¤çš„è“ç‰™åå­?
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    switch (handle) {
#if 1	 //------------------------------------------------------@20240224 ATT_Sang Ë«Ïò²âÊÔ ÌØÕ÷×Ö·û´®¿ªÊ¼-------------------------------------------------------
	case ATT_CHARACTERISTIC_FEC9_01_VALUE_HANDLE:
		/////ĞèÒª·µ»ØµÄ³¤¶È
	  const u8 test_read_buf_sangzijin[5]={'s','a','n','2',0x55};
...
static int make_set_adv_data(void)
{
    ...
    //Company IDï¼?2 bytesï¼? + å‚å•†IDï¼?2 Bytesï¼?+ æ³¨å†ŒçŠ¶æ?ï¼ˆ1byteï¼?+ MAC(6 bytes)
...
#define PASSKEY_ENTER_ENABLE      0 //è¾“å…¥passkeyä½¿èƒ½ï¼Œå¯ä¿®æ”¹passkey
//é‡è®¾passkeyå›è°ƒå‡½æ•°ï¼Œåœ¨è¿™é‡Œå¯ä»¥é‡æ–°è®¾ç½®passkey
//passkeyä¸?6ä¸ªæ•°å­—ç»„æˆï¼Œåä¸‡ä½ã?ä¸‡ä½ã?‚ã?‚ã?‚ã?‚ä¸ªä½? å„è¡¨ç¤ºä¸€ä¸ªæ•°å­? é«˜ä½ä¸å¤Ÿä¸?0
static void reset_passkey_cb(u32 *key)
{
#if 1
    u32 newkey = rand32();//è·å–éšæœºæ•?

    newkey &= 0xfffff;
    if (newkey > 999999) {
        newkey = newkey - 999999; //ä¸èƒ½å¤§äº999999
    }
    *key = newkey; //å°äºæˆ–ç­‰äºå…­ä½æ•°
