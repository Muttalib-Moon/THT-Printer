...
//#define CONFIG_SDCARD_DEBUG_ENABLE
//#define CONFIG_NETWORK_DEBUG_ENABLE
//#define CONFIG_USB_DEBUG_ENABLE
...
static char lg_buf[DATA_BUF_SIZE];
static u8 write_index;
static u8 write_busy;
...
static void sd_log_task(void *priv)
{
    ...
    while (1) {
        if (!storage_device_ready()) { //SD卡未就绪
            ...
            if (write_busy && last_write_time == write_index) { //�?要写入数据并且数据包和上�?包一�?
...
static char lg_buf[DATA_BUF_SIZE]; //打印信息缓存�?
...
void putbyte(char c)
{
    ...
    lg_buf[t] = c;
    if (!first_write && !(t % WRITE_SIZE)) {//判断t为WRITE_SIZE的整数�??
        write_busy = 1;//标记�?要写入数�?
        write_index = t / WRITE_SIZE;//write_index只为0�?1,�?1时是第一包数�?,�?0时是第二包数�?
...
int user_putchar(int a)//用户的user_printf 的底层发送函�?
{
    putbyte(a);//可以换用户的串口发�?�函�?
    return a;
}
#endif
