...
const char *bt_get_local_name(void)
{
    ...
    if (edr_name[0] == 0) { // not yet set
        if (syscfg_read(CFG_BT_NAME, edr_name, sizeof(edr_name)) <= 0) {
            const u8 *mac_addr = bt_get_mac_addr();
            sprintf(edr_name, "JL-AC79XX-%02X%02X%02X",mac_addr[5], mac_addr[4], mac_addr[3]);
        }
    }
#endif
    return edr_name;
