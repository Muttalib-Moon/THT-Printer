...
#define FORCE_DEFAULT_MODE 0 //配置wifi_on之后的模�?,0为使用最后记忆的模式, 1为强制默认模�?, 3-200为连接超时时间多少秒,如果超时都连接不上就连接�?后记忆的或�?�最优网�?
#endif
#define INIT_STORED_SSID (INIT_MODE==STA_MODE)//配置STA模式情况�?,把默认配置SSID也存储起�?, 以后即使保存过其他SSID,也不会覆盖丢�?,使用连接�?优信号SSID策略的情况下可以匹配连接
#define INIT_CONNECT_BEST_SSID 1 //配置如果啟動WIFI后在STA模式�?, 是否挑�?�连接记忆过的信号最优WIFI
...
void wifi_sta_connect(char *ssid, char *pwd, char save, void *priv)
{
    if (save) {
        ...
        if (strcmp(info.ssid, ssid) || strcmp(info.pwd, pwd)) {
            __this->save_ssid_flag = 1;    //防止输错密码还保存ssid, 等待确保连接上路由器再保�?
        }
    }
...
static void wifi_app_task(void *priv)  //主要是create wifi 线程�?
{
...
static int wireless_net_init(void)   //主要是create wifi 线程�?
{
