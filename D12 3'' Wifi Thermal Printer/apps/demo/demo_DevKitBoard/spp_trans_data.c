...
/*
 打开流控使能�?,确定配置接口 transport_spp_flow_cfg 被调用初始化配置
 然后使用过程 通过接口 transport_spp_flow_enable 来控制流控开�?
 */
#define SPP_DATA_RECIEVT_FLOW              0//流控功能使能

//流控速度控制，可以不用更�?
#define FLOW_SEND_CREDITS_NUM              1 //控制命令�? 控制可接收的数据包个�?,发�?�给对方�?,range(1~32)
#define FLOW_SEND_CREDITS_TRIGGER_NUM      1 //触发更新控制命令的阈�?,range(1 to <= FLOW_SEND_CREDITS_NUM)
