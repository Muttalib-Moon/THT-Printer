...
/****************SD模块**********************************************************
 * 暂只支持1个SD外设，如�?要多个SD外设，自行去掉下面SD0/1_ENABLE控制，另外定义即�?
********************************************************************************/
#if TCFG_SD0_ENABLE
//sd0
...
const struct rdec_platform_data rdec_key_data = {
    .enable = 1,
    .num = ARRAY_SIZE(rdeckey_list),            //RDEC按键的个�?
    .rdec = rdeckey_list,                       //RDEC按键参数�?
};
#endif
...
static const struct iokey_port iokey_list[] = {
    {
        .connect_way = ONE_PORT_TO_LOW,         //IO按键的连接方�?
        .key_type.one_io.port = IO_PORTA_07,    //IO按键对应的引�?
        .key_value = KEY_UP,                    //按键�?
    },
    {
        .connect_way = ONE_PORT_TO_LOW,
        .key_type.one_io.port = IO_PORTA_08,
        .key_value = KEY_DOWN,                  //按键�?
    },
    {
        .connect_way = ONE_PORT_TO_LOW,
        .key_type.one_io.port = IO_PORTB_01,
        .key_value = KEY_MODE,                  //按键�?
    },
};
const struct iokey_platform_data iokey_data = {
    .enable = 1,                              //是否使能IO按键
    .num = ARRAY_SIZE(iokey_list),            //IO按键的个�?
    .port = iokey_list,                       //IO按键参数�?
};
#endif
...
const struct touch_key_platform_data touch_key_data = {
    ...
    .change_gain 	= 4,		//变化放大倍数, �?般固�?
    .press_cfg		= -100,		//触摸按下灵敏�?, 类型:s16, 数�?�越�?, 灵敏度越�?
    .release_cfg0 	= -50,		//触摸释放灵敏�?0, 类型:s16, 数�?�越�?, 灵敏度越�?
    .release_cfg1 	= -80,		//触摸释放灵敏�?1, 类型:s16, 数�?�越�?, 灵敏度越�?
...
static const struct plnk_platform_data plnk0_data = {
    ...
    .share_data_io = 1, //两个数字MIC共用�?个DAT�?
    .high_gain = 1,
...
PWM_PLATFORM_DATA_BEGIN(pwm_data0)
    .port   = IO_PORTC_01,
    .pwm_ch = PWM_TIMER3_OPCH3 | PWMCH4_L,//初始化可选多通道,�?:PWMCH0_H | PWMCH0_L | PWMCH1_H ... | PWMCH7_H | PWMCH7_L | PWM_TIMER2_OPCH2 | PWM_TIMER3_OPCH3 ,
    .freq   = 200,//频率
    .duty   = 90,//占空�?
...
PWM_PLATFORM_DATA_BEGIN(pwm_data1)
    .port   = IO_PORTA_01,//选择定时器的TIMER PWM任意IO，pwm_ch加上PWM_TIMER3_OPCH3或PWM_TIMER2_OPCH2有效,只支�?2个PWM,占用output_channel2/3，其他外设使用output_channel�?留意
    .pwm_ch = PWM_TIMER2_OPCH2,//初始化可选多通道,�?:PWMCH0_H | PWMCH0_L | PWMCH1_H ... | PWMCH7_H | PWMCH7_L | PWM_TIMER2_OPCH2 | PWM_TIMER3_OPCH3 ,
    .freq   = 100000,//频率
    .duty   = 50,//占空�?
    .point_bit = 0,//根据point_bit值调节占空比小数点精度位: 0<freq<=4K,point_bit=2;4K<freq<=40K,point_bit=1; freq>40K,point_bit=0;
PWM_PLATFORM_DATA_END()
...
const struct wifi_calibration_param wifi_calibration_param = {
    .xosc_l     = 0xb,// 调节左晶振电�?
    .xosc_r     = 0xb,// 调节右晶振电�?
    .pa_trim_data  ={1, 7, 4, 7, 11, 1, 7},// 根据MP测试生成PA TRIM�?
	.mcs_dgain     ={
		50,//11B_1M
	  	50,//11B_2.2M
	  	50,//11B_5.5M
...
static const struct low_power_param power_param = {
    ...
    .vddiom_lev     = TCFG_LOWPOWER_VDDIOM_LEVEL,          //强VDDIO等级,可�?�：2.2V  2.4V  2.6V  2.8V  3.0V  3.2V  3.4V  3.6V
    .vddiow_lev     = TCFG_LOWPOWER_VDDIOW_LEVEL,          //弱VDDIO等级,可�?�：2.1V  2.4V  2.8V  3.2V
    .vdc14_dcdc 	= TRUE,	   							   //打开内部1.4VDCDC，关闭则用外�?
    .vdc14_lev		= VDC14_VOL_SEL_LEVEL, 				   //VDD1.4V配置
    .sysvdd_lev		= SYSVDD_VOL_SEL_LEVEL,				   //内核、sdram电压配置
	.vlvd_enable	= TRUE,                                //TRUE电压复位使能
	.vlvd_value		= VLVD_SEL_25V,                        //低电压复位电压�??
};

/************************** PWR config ****************************/
//#define PORT_VCC33_CTRL_IO		IO_PORTA_03					//VCC33 DCDC控制引脚,该引脚控制DCDC器件输出�?3.3V连接芯片HPVDD、VDDIO、VDD33
#define PORT_WAKEUP_IO			IO_PORTB_01					//软关机和休眠唤醒引脚
#define PORT_WAKEUP_NUM			(PORT_WAKEUP_IO/IO_GROUP_NUM)//默认:0-7:GPIOA-GPIOH, 可以指定0-7�?

static const struct port_wakeup port0 = {
    .edge       = FALLING_EDGE,                            //唤醒方式选择,可�?�：上升沿\下降�?
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = PORT_WAKEUP_IO,                          //唤醒口�?�择
...
static const struct long_press lpres_port = {
    ...
    .edge		= FALLING_EDGE,								//长按方式,可�?�：FALLING_EDGE /  RISING_EDGE --> 低电�?/高电�?
    .iomap 		= PORT_WAKEUP_IO,							//长按复位IO和IO唤醒共用�?个IO
};
...
/*进软关机之前默认将IO口都设置成高阻状态，�?要保留原来状态的请修改该函数*/
static void board_set_soft_poweroff(void)
{
    ...
    for (u8 p = 0; p < 8; ++p) {
        //flash sdram PD PE PF PG口不能进行配�?,由内部完成控�?
