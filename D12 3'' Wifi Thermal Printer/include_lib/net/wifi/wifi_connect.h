...
// @brief 配置WIFI MP测试后校准后�? 晶体频偏参数 ,PA参数 , 各个速率功率的数字增益参�?
...
/**
 * @brief wifi_set_frame_cb，用于注册（设置）WIFI底层接收�?802.11数据帧回调函�?
 *
 * @param cb 指向WIFI底层接收�?802.11数据帧回调函�?
 * @param priv  用户私有指针传�??
 */
 ...
 *
 * @param pwr_sel 默认�?6，范围为0-6
 *
 * @note 参数设为0，表示把WIFI模拟功率调整到最低档位节�?
 */
 ...
 *
 * @param mac 指向存储MAC地址的缓存数组，数组大小�?6
 */
 ...
 *         |1|AP模式默认不过滤|
 *         |2|STA模式下使�?,过滤广播,多播|
 *         |3|STA模式下使�?,过滤not_my_bssid|
 *         |4|STA模式下使�?,过滤广播+多播+not_my_bssid|
 *         |5|AP模式下使�?,过滤广播,多播|
 *         |6|AP模式下使�?,过滤not_my_bssid|
 *         |7|AP模式下使�?,过滤广播+多播+not_my_bssid|
 */
 ...
 *
 * @param tab 速率表，不需要哪个�?�率就删除掉,可以动�?�设�?
 */
...
/**
 * @brief wifi_get_mode_cur_info，用于获取WIFI当前是什么模�?,或�?�当前指定模式下的配置信�?
 *
 * @param info 指向�?个wifi_mode_info类型的结构体，其包含了变量mode、ssid、pwd
 */
extern void wifi_get_mode_cur_info(struct wifi_mode_info *info);

/**
 * @brief wifi_get_mode_stored_info，用于获取WIFI�?后记忆的是什么模�?,或�?�最后记忆模式下的配置信�?
 *
 * @param info 指向�?个wifi_mode_info类型的结构体，其包含了变量mode、ssid、pwd
 */
 ...
 *
 * @param parm 指向�?个wifi_store_info类型的结构体
 * @param force 配置wifi_on之后的模�?
 *         |force|说明|
 *         |- |- |
 *         |0|使用�?后记忆的模式|
 *         |1|强制默认模式|
 *         |3-200|STA连接超时时间多少�?,如果超时都连接不上就连接�?后记忆的或�?�最优网络|
 ...
 *         |1|存储默认配置的SSID|
 *
 * @note 配置STA模式情况�?,把默认配置SSID也存储起�?,以后即使保存过其他SSID,也不会覆盖丢�?,使用连接�?优信号SSID策略的情况下可以匹配连接
 */
...
/**
 * @brief wifi_set_target_mac_for_agc_recv，用于设置目标接收地�?,有利于提高抗干扰能力,专门针对目标地址的数据包进行接收AGC和频偏调�?
 *
 * @param mac 目标接收地址
 */
...
/**
 * @brief wifi_disconnect_station，用于AP模式下断�?指定MAC地址的STA
 *
 * @param mac 要断�?设备的mac地址
 * @param reason 断开的原因，通常�?8:Deauthenticated because sending station is leaving
 */
...
/**
 * @brief wifi_set_store_ssid_cnt，用于设置WIFI�?多保存多少个连接的SSID数目
 *
 * @param cnt sta模式下最多保存的SSID数目，工程中默认�?5
 */
extern void wifi_set_store_ssid_cnt(u32 cnt);

/**
 * @brief wifi_set_sta_connect_best_ssid，用于设置WIFI进入STA模式的时候如果匹配到信号�?好的网络就去连接
 *
 * @param enable 自动连接保存过的信号�?好的SSID使能�?,�?1：使能，�?0：不使能
 */
 ...
 *
 * @param sec WIFI连接STA超时时间，单位为秒，工程中默认为60�?
 */
extern void wifi_set_sta_connect_timeout(int sec);

/**
 * @brief wifi_set_connect_sta_block，用于设置WIFI连接STA是否阻塞, 默认非阻塞�?�过事件通知
 *
 * @param block 使能端，�?1：阻塞，置为0：不阻塞
 */
 ...
 * @param sta_pwd 配置STA模式的密�?
 */
extern int wifi_enter_sta_mode(char *sta_ssid, char *sta_pwd);

/**
 * @brief wifi_scan_req，用于WIFI STA模式或�?�AP模式下启动一次扫描空中SSID
 * @note STA模式下调用返�?-1代表WIFI OFF/正在扫描/连接�?,无需启动扫描, 可以等待几秒或�?�扫描完成事件到来获取结�?
 */
extern int wifi_scan_req(void);

/**
 * @brief wifi_get_scan_result，用于启动一次扫描空中SSID�?,获取扫描结果
 */
 ...
 *
 * @note 返回WIFI连接的STA通信丢包质量,0-100,�?般认为大�?50为较�?,20-50之间为一�?,小于20较差, decide ChannelQuality based on: 1)last BEACON received time, 2)last RSSI权重50, 3)TxPER权重30, and 4)RxPER权重20
 */
extern char wifi_get_cqi(void);

/**
 * @brief wifi_get_sta_connect_state，用于获取WIFI是连接STA状�??
 *
 * @note 返回WIFI连接STA的状�?
 */
 ...
 *
 * @param sec WIFI配网超时时间，单位为�?
 */
extern void wifi_set_smp_cfg_timeout(int sec);

/**
 * @brief wifi_set_smp_cfg_scan_all_channel，用于设置WIFI配网模式/monitor模式下是否扫描全部信�?,默认�?
 *
 * @param onoff 全扫描开关，1:�?启，0:关闭
 */
extern void wifi_set_smp_cfg_scan_all_channel(char onoff);

/**
 * @brief wifi_set_smp_cfg_airkiss_recv_ssid，用于设置WIFI airkiss配网模式下是否接收完整的SSID,默认�?,有助于加快配网时�?,有概率配网失�?
 *
 * @param onoff 全扫描开关，1:�?启，0:关闭
 */
...
/**
 * @brief wifi_set_smp_cfg_just_monitor_mode，用于设置WIFI进入配网/monitor模式�?,是否只保留monitor模式,默认�?
 *
 * @param onoff 全扫描开关，1:�?启，0:关闭
 */
...
/**
 * @brief wifi_set_monitor_mode_scan_channel_time，用于在WIFI只保留monitor模式的情况下,设置扫描每个信道的时间间�?
 *
...
/**
 * @brief wifi_send_data，用于使用WIFI底层接口直接发�?�数据帧
 *
 * @param pkg 要发送的数组
 * @param len 发�?�数据的长度,�?�?2510字节
 * @param rate WIFI发�?��?�率
 */
...
/**
 * @brief wifi_get_remain_tx_queue，用于获取WIFI底层发�?�队列剩余个�?
 *
 * @param ac_queue 为Qos优先�?,用户无配置就是填0
 *
 * @return 剩余多少个包可填�?,�?大为 MAX_PACKETS_IN_QUEUE
 */
extern u32 wifi_get_remain_tx_queue(char ac_queue);


/**
 * @brief wifi �?大backoff范围设置
 *
