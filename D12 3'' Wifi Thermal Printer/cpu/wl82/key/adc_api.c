...
static void adc_isr()
{
    ...
    // Store current channel result before moving to next channel
    switch(cur_ch) {
        case 0: // AD_CH_PA08 (Channel 1) - KeyADC
                ADCValue[0] = _adc_res;
                adc_queue[0].value = _adc_res;
                break;
        case 1: // AD_CH_PA10 (Channel 2) - TMO
                ADCValue[1] = _adc_res;
                adc_queue[1].value = _adc_res;
                break;
        case 2: // AD_CH_PB07 (Channel 5) - Paper R
                ADCValue[2] = _adc_res;
                adc_queue[2].value = _adc_res;
                break;
        case 3: // AD_CH_PC01 (Channel 7) - Gap R
                ADCValue[3] = _adc_res;
                adc_queue[3].value = _adc_res;
                break;
    }

    // Get next channel in sequence
    next_ch = (cur_ch + 1) % 4; // Cycle through 0,1,2,3

    // Start conversion for next channel
    switch(next_ch) {
        case 0: // AD_CH_PA08
                adc_sample(AD_CH_PA08);
                break;
        case 1: // AD_CH_PA10
                adc_sample(AD_CH_PA10);
                break;
        case 2: // AD_CH_PB07
                adc_sample(AD_CH_PB07);
                break;
        case 3: // AD_CH_PC01
                adc_sample(AD_CH_PC01);
                break;
    }
    cur_ch = next_ch;
...
static void __adc_init(void)
{
	...
	/*adc_add_sample_ch(AD_CH_PA07); // KeyAdc---ADC value for key board-->channel 0
	adc_add_sample_ch(AD_CH_PA08); // PaperAdc--ADC value for edge paper sensor-->channel 1
	adc_add_sample_ch(AD_CH_PB01); // ADC value for woking power on-off	-->channel 3
	adc_add_sample_ch(AD_CH_PB07); // GAP sensor detection value-->channel 5
	adc_add_sample_ch(AD_CH_PC01); // Print header temparature ADC value-->channel 7
	adc_add_sample_ch(AD_CH_PC09); // USB power ADC value-->channel 8*/

	adc_add_sample_ch(AD_CH_PA08); //KeyADC 	PA8 (Channel:1 )
	adc_add_sample_ch(AD_CH_PA10); //TMO        PA10(Channel:2 )
	adc_add_sample_ch(AD_CH_PB07); //Paper R	PB7 (Channel:5 )
	adc_add_sample_ch(AD_CH_PC01); //Gap R	    PC1 (Channel:7 )
